  ...
  return (
    <AppLayout>
      ...
      <div className="space-y-5">
        ...
        {/* ── WALLET ── */}
        <section className="glow-card glass-card-shimmer rounded-2xl p-5 md:p-6 relative overflow-hidden border border-white/[0.08] bg-white/[0.03] backdrop-blur-3xl shadow-[inset_0_1px_0_rgba(255,255,255,0.08),0_8px_40px_rgba(0,0,0,0.30)]">
          ...
          {walletAssets.length > 0 && (
            <div className="relative mt-4 flex gap-2 overflow-x-auto pb-1 -mx-1 px-1">
              {walletAssets.slice(0, 10).map((b) => (
                <div key={b.asset} className="glass-subcard-shimmer shrink-0 rounded-xl border border-white/[0.07] bg-white/[0.02] backdrop-blur-2xl px-3 py-2 flex items-center gap-2 min-w-[150px] hover:border-primary/40 transition-colors shadow-[inset_0_1px_0_rgba(255,255,255,0.06)] overflow-hidden">
        ...
        {/* ── ACTIVE TRADE ── */}
        <section className={`glass-card-shimmer rounded-2xl border bg-white/[0.03] backdrop-blur-3xl p-5 md:p-6 relative overflow-hidden transition-shadow shadow-[inset_0_1px_0_rgba(255,255,255,0.08),0_8px_40px_rgba(0,0,0,0.30)] ${primary ? "border-primary/20 shadow-[inset_0_1px_0_rgba(255,255,255,0.08),0_0_60px_-20px_rgba(94,234,212,0.45)]" : "border-white/[0.08]"}`}>
          {primary ? (
            <>
              ...
              <div className={`glass-subcard-shimmer relative mt-5 rounded-xl border backdrop-blur-2xl bg-white/[0.02] px-4 py-3 flex items-center justify-between transition-all duration-300 shadow-[inset_0_1px_0_rgba(255,255,255,0.06)] overflow-hidden ${flash === "up" ? "border-bull/50 !bg-bull/[0.07]" : flash === "down" ? "border-bear/50 !bg-bear/[0.07]" : "border-white/[0.07]"}`}>
              ...
              {showDca && (
                <div
                  ...
                  style={{
                    background: "linear-gradient(135deg, color-mix(in oklab,var(--primary) 6%,rgba(255,255,255,0.015)) 0%, color-mix(in oklab,var(--primary) 3%,rgba(255,255,255,0.008)) 100%)",
                    border: "1px solid color-mix(in oklab,var(--primary) 22%,rgba(255,255,255,0.06))",
                    backdropFilter: "blur(32px)",
                    WebkitBackdropFilter: "blur(32px)",
                    boxShadow: "inset 0 1px 0 rgba(255,255,255,0.07), 0 0 24px -8px color-mix(in oklab,var(--primary) 28%,transparent)",
                  }}
                >
                  <div
...
function Cell({ label, value, accent, danger }: { label: string; value: string; accent?: boolean; danger?: boolean }) {
  return (
    <div className={`glass-subcard-shimmer rounded-lg border px-3 py-2 backdrop-blur-2xl overflow-hidden shadow-[inset_0_1px_0_rgba(255,255,255,0.05)] ${danger ? "border-bear/20 bg-bear/[0.07]" : accent ? "border-bull/20 bg-bull/[0.07]" : "border-white/[0.07] bg-white/[0.02]"}`}>
...
function ProgressTrack({ icon, label, fromLabel, toLabel, pct, rightValue, hint, color }: {
  ...
  return (
    <div className="glass-subcard-shimmer rounded-xl border border-white/[0.07] bg-white/[0.02] backdrop-blur-2xl p-3 relative overflow-hidden shadow-[inset_0_1px_0_rgba(255,255,255,0.06)]">
      <style>{`
