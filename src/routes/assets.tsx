...
import { getAccount, getAllPrices, getTickers24h } from "@/lib/binance.functions";
import { useMemo } from "react";

export const Route = createFileRoute("/assets")({
  head: () => ({
    meta: [
      { title: "Assets — CryptoHunt" },
      { name: "description", content: "Your real Binance Spot balances with live USD values and 24h change." },
    ],
  }),
...
function AssetsPage() {
  ...
  const prices = useQuery({ queryKey: ["prices"], queryFn: () => getAllPrices(), refetchInterval: 8_000 });

  const symbols = useMemo(() => {
    const list = account.data?.balances.filter((b) => b.asset !== "USDT").map((b) => `${b.asset}USDT`) ?? [];
    return list;
  }, [account.data]);
    ...
    <AppLayout>
      <div className="space-y-5">
        <section className="grid gap-4 md:grid-cols-3">
          ...
          <Tile
            label="24h Change"
            value={`${totalChangeUsd >= 0 ? "+" : ""}$${fmt(totalChangeUsd)}`}
            sub={`${totalUsd > 0 ? ((totalChangeUsd / totalUsd) * 100).toFixed(2) : "0"}%`}
            tone={totalChangeUsd >= 0 ? "bull" : "bear"}
          />
          <Tile label="Active Holdings" value={String(rows.length)} sub="With non-zero balance" tone="primary" />
        </section>
              ...
              <tbody>
                {rows.map((r) => (
                  <tr key={r.asset} className="border-t border-border/60">
                    ...
                    <td className={`text-right px-3 py-3 font-mono ${r.locked > 0 ? "text-bear" : "text-muted-foreground"}`}>
                      {fmt(r.locked, 6)}
                    </td>
