...
import { getOpenOrders, getAllPrices } from "@/lib/binance.functions";

export const Route = createFileRoute("/trade")({
  head: () => ({
    meta: [
      { title: "Trade — CryptoHunt" },
      { name: "description", content: "Track your open Binance order with live price, P&L and chart." },
    ],
  }),
  component: TradePage,
});
...
function TradePage() {
  ...
  useEffect(() => {
    if (!selected && orders.data && orders.data.length > 0) {
      setSelected(String(orders.data[0].orderId));
    }
  ...
  if (orders.isLoading) {
    return (
      <AppLayout>
        <div className="text-muted-foreground">Loading your open orders…</div>
      </AppLayout>
    );
  }
    ...
    return (
      <AppLayout>
        <div className="rounded-2xl border border-border bg-card p-10 text-center">
          ...
          <p className="text-sm text-muted-foreground mt-2">When you have an active limit order, it will appear here with a live chart.</p>
        </div>
      </AppLayout>
    );
  }
    ...
    <AppLayout>
      <div className="space-y-5">
          ...
          <div className="flex gap-2 flex-wrap">
            {orders.data.map((o) => {
              ...
              return (
                <button
                  key={o.orderId}
                  onClick={() => setSelected(String(o.orderId))}
                  className={`flex items-center gap-2 px-3 py-2 rounded-lg border text-sm font-bold transition-colors ${
                    active ? "border-primary/60 bg-primary/10 text-primary" : "border-border bg-card text-muted-foreground hover:text-foreground"
                  }`}
                >
                  <CoinIcon symbol={b} size={20} />
                  {o.symbol}
                </button>
              );
            })}
        ...
        <section className="rounded-2xl border border-border bg-card p-5 md:p-6">
          <div className="grid grid-cols-[minmax(0,1fr)_auto] sm:flex sm:flex-wrap items-start gap-4 sm:justify-between">
